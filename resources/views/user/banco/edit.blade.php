...
@section('content')
    ...
    <div class="container-xl px-4 mt-4">
        ...
        <hr class="mt-0 mb-4" />
        <form action="{{route('user.banco.update', $lancBanco->id)}}" method="POST">
                                    ...
                                    <div class="row gx-3 mb-3">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <input class="form-control" id="data" name="data" type="date"
                                                placeholder="Enter your first name" value="{{$lancBanco->data}}" />
                                        </div>
                                        <!-- Form Group (last name)-->
                                        <div class="col-md-2">
                                            <label class="small mb-1" for="inputLastName">Entrada/Saída</label>
                                            <select class="form-control" name="tipo" id="tipo">
                                                <option value="entrada" {{ $lancBanco->tipo == 'entrada' ? 'selected' : '' }}>Entrada</option>
                                                <option value="saida" {{ $lancBanco->tipo == 'saida' ? 'selected' : '' }}>Saída</option>
                                            </select>
                                        </div>
                                    ...
                                    <div class="row gx-3 mb-3">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <select class="form-select " name="tipo_documento" id="tipo_documento">
                                                <option value="OUTR - Dafe" {{ $lancBanco->tipo_documento == 'OUTR - Dafe' ? 'selected' : '' }}>OUTR - Dafe</option>
                                                <option value="NF - Nota Fiscal" {{ $lancBanco->tipo_documento == 'NF - Nota Fiscal' ? 'selected' : '' }}>NF - Nota Fiscal</option>
                                                <option value="DANF - Danfe" {{ $lancBanco->tipo_documento == 'DANF - Danfe' ? 'selected' : '' }}>DANF - Danfe</option>
                                                <option value="BOL - Boleto" {{ $lancBanco->tipo_documento == 'BOL - Boleto' ? 'selected' : '' }}>BOL - Boleto</option>
                                                <option value="REP - Repasse" {{ $lancBanco->tipo_documento == 'REP - Repasse' ? 'selected' : '' }}>REP - Repasse</option>
                                                <option value="CCRD - Cartão de Credito" {{ $lancBanco->tipo_documento == 'CCRD - Cartão de Credito' ? 'selected' : '' }}>CCRD - Cartão de Credito</option>
                                                <option value="CDBT - Cartào de Debito" {{ $lancBanco->tipo_documento == 'CDBT - Cartào de Debito' ? 'selected' : '' }}>CDBT - Cartào de Debito</option>
                                                <option value="CH - Cheque" {{ $lancBanco->tipo_documento == 'CH - Cheque' ? 'selected' : '' }}>CH - Cheque</option>
                                                <option value="REC - Recibo" {{ $lancBanco->tipo_documento == 'REC - Recibo' ? 'selected' : '' }}>REC - Recibo</option>
                                                <option value="CARN - Carnê" {{ $lancBanco->tipo_documento == 'CARN - Carnê' ? 'selected' : '' }}>CARN - Carnê</option>
                                                <option value="FAT - Fatura" {{ $lancBanco->tipo_documento == 'FAT - Fatura' ? 'selected' : '' }}>FAT - Fatura</option>
                                                <option value="APOL - Apólice" {{ $lancBanco->tipo_documento == 'APOL - Apólice' ? 'selected' : '' }}>APOL - Apólice</option>
                                                <option value="DUPL - Duplicata" {{ $lancBanco->tipo_documento == 'DUPL - Duplicata' ? 'selected' : '' }}>DUPL - Duplicata</option>
                                                <option value="TRIB - Tribunal" {{ $lancBanco->tipo_documento == 'TRIB - Tribunal' ? 'selected' : '' }}>TRIB - Tribunal</option>
                                                <option value="T Banc - Transferêrencia Bancaria" {{ $lancBanco->tipo_documento == 'T Banc - Transferêrencia Bancaria' ? 'selected' : '' }}>T Banc - Transferêrencia Bancaria</option>
                                            </select>
                                        </div>
                                        <!-- Form Group (birthday)-->
                                        <div class="col-md-4">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <div class="input-group input-group-joined">
                                                ...
                                                name="valor" value=" {{ number_format($lancBanco->valor, 2, ',', '.') }}" required placeholder="valor" aria-label="">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row gx-3 mb-3">
                                        <div class="col-md-12">
                                            ...
                                            <textarea class="form-control"
                                                        ...
                                                        id="complemento"
                                                        cols="20"
                                                        rows="3">{{$lancBanco->complemento}}</textarea>
                                        </div>
                                    </div>
                                    <!-- Save changes button-->
                                </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <a class="text-arrow-icon small" href="{{route('user.banco.list')}}">
                    ...
                    <input class="form-control" id="inputNotificationEmail" type="email" value="{{$lancBanco->id}}"
                        disabled />
                </div>
            </div>
        </div>
